...
int main() 
{
    ...
    if (counter == MAP_FAILED) err_msg("mmap");

    *counter = 0;

    sem = (sem_t*)mmap(NULL, sizeof(sem_t), PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_SHARED, -1, 0);

    if (sem == MAP_FAILED) err_msg("mmap");

    if (sem_init(sem, 1, 1) == -1) err_msg("sem_init");
    ...
    int status;
    if (wait(&status) == (pid_t) - 1) err_msg("wait");

    printf(WIFEXITED(status) ? "Child returned successfully\n" : "Child returned not successfully\n");

    if (sem_destroy(sem) == -1) err_msg("sem_destroy");

    if (munmap(counter, sizeof(int)) == -1) err_msg("munmap");
    if (munmap(sem, sizeof(sem_t)) == -1) err_msg("munmap");
}
