...
void *createKey() 
{
    if (pthread_key_create(&key, destructor)) err_msg("pthread_key_create");
}

char *pthread_strerror(int arg) 
{
    if (pthread_once(&pth_once, createKey)) err_msg("pthread_once");

    char *msg;
    if (!(msg = pthread_getspecific(key))) 
    {
        if (!(msg = malloc(BUF_SIZE))) err_msg("malloc");
        if (pthread_setspecific(key, msg)) err_msg("pthread_setspecific");
    }
...
int main() 
{
    ...
    if (pthread_create(&pthread3, NULL, threadFunc, (void*)3)) err_msg("pthread_create");

    printf("%s\n", pthread_strerror(0));

    if (pthread_join(pthread1, NULL)) err_msg("pthread_join");
    if (pthread_join(pthread2, NULL)) err_msg("pthread_join");
    if (pthread_join(pthread3, NULL)) err_msg("pthread_join");
}