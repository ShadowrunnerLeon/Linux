...
int main() 
{
    ...
    if (sigemptyset(&sa.sa_mask) == -1) err_msg("sigemptyset");
    if (sigaction(SIGCONT, &sa, NULL) == -1) err_msg("sigaction");

    sigset_t sigmask;
    if (sigemptyset(&sigmask) == -1) err_msg("sigemptyset");
    if (sigaddset(&sigmask, SIGCONT) == -1) err_msg("sigaddset");
