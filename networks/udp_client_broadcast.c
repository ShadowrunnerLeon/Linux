...
int main() 
{
    ...
    if ((sockfd = socket(AF_INET, SOCK_DGRAM, 0)) == -1) err_msg("socket");

    int broad_flag = 1;
    if (setsockopt(sockfd, SOL_SOCKET, SO_BROADCAST,
        &broad_flag, sizeof broad_flag) == -1) err_msg("setcockopt");
    ...
    int numbytes;
    if ((numbytes = sendto(sockfd, "Broadcast packet", 17, 0,
        (struct sockaddr*)&broad_struct, sizeof broad_struct)) == -1) err_msg("sendto");
